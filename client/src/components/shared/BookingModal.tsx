      ...
      <div className="bg-white w-full sm:w-96 max-h-[85vh] rounded-t-2xl sm:rounded-2xl overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-border">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{tripItemTitle}</p>
          </div>
          <button
            onClick={onClose}
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <h4 className="font-bold text-foreground line-clamp-1">{option.name}</h4>
                        {selectedOption?.id === option.id && (
                          <Check className="h-5 w-5 text-primary shrink-0" />
                        )}
                      </div>
                      {option.vendorName && (
                        <p className="text-xs text-muted-foreground">{option.vendorName}</p>
                      )}
                      {option.description && (
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="rounded-full bg-white border border-border p-1.5 hover:bg-muted"
                  disabled={guestCount <= 1}
                  data-testid="button-decrease-guests"
                >
                  <Minus className="h-4 w-4 text-muted-foreground" />
                </button>
                <span className="font-bold text-foreground w-8 text-center">{guestCount}</span>
                <button
